...
const HeroBand = memo(function HeroBand({
  ...
  // Store initial prop values in a ref so the setup effect doesn't need them as dependencies.
  // The second useEffect handles all prop updates after initialization.
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uTime: { value: 0 },
        uSpeed: { value: initProps.speed },
        uRot: { value: new THREE.Vector2(1, 0) },
        uColor: { value: new THREE.Vector3(0.66, 0.33, 0.97) },
        uScale: { value: initProps.scale },
        uFrequency: { value: initProps.frequency },
        uWarpStrength: { value: initProps.warpStrength },
        uNoise: { value: initProps.noise },
        uBandWidth: { value: initProps.bandWidth },
        uYOffset: { value: initProps.yOffset },
        uFadeTop: { value: initProps.fadeTop },
        uPointer: { value: new THREE.Vector2(0, 0) },
        uMouseInfluence: { value: initProps.mouseInfluence },
        uIterations: { value: initProps.iterations },
        uIntensity: { value: initProps.intensity },
      },
  ...
  // Sync prop changes into the existing material uniforms without re-creating the WebGL context
  useEffect(() => {
